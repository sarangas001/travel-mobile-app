...
export default function DestinationDetailScreen() {
  ...
  const destination = MOCK_DESTINATIONS.find((d) => d.id === id) || MOCK_DESTINATIONS[0];

  // Component states
  const [activeTab, setActiveTab] = useState<'Overview' | 'Promo' | 'Review'>('Overview');
  ...
  const selectedDate = destination.availabilityCalendar.find(c => c.id === selectedDateId);
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      >
        {/* Cover Photo Header */}
        <View style={{ height: SCREEN_HEIGHT * 0.46 }} className="w-full relative overflow-hidden bg-peach-light/45">
          <Image
          ...
          <SafeAreaView edges={['top']} className="absolute inset-x-0 top-0 flex-row justify-between items-center px-6 py-2">
            {/* Back Button */}
            <Pressable 
              onPress={() => router.back()}
        ...
        <View className="bg-white rounded-t-[36px] -mt-6 px-6 pt-8 pb-4">
          ...
          <View className="flex-row gap-8 border-b border-gray-100 pb-3 mb-5">
            {(['Overview', 'Promo & Discount', 'Review'] as const).map((tab) => {
              const tabId = tab.startsWith('Overview') ? 'Overview' : tab.startsWith('Promo') ? 'Promo' : 'Review';
              ...
              return (
                ...
                >
                  <Text className={`text-base font-bold transition-colors ${isActive ? 'text-brand-navy' : 'text-gray-sub'}`}>
                    {tab}
                  </Text>
                  {isActive && (
          ...
          {activeTab === 'Promo' && (
            <View className="mb-6 p-4 bg-green-50/50 rounded-2xl border border-green-100 flex-row items-center gap-3">
              ...
              <View className="flex-1">
                <Text className="text-sm font-bold text-brand-navy">20% Summer Early Bird Discount</Text>
                <Text className="text-xs text-gray-sub">Applies automatically for reservations this week.</Text>
              </View>
            </View>
          )}
          ...
          {activeTab === 'Review' && (
            <View className="mb-6 gap-4">
              {destination.reviews.map((rev) => (
                <View key={rev.id} className="p-4 bg-bg-gray rounded-2xl border border-gray-50">
                  <View className="flex-row justify-between items-center mb-2">
                    <View className="flex-row items-center gap-2.5">
                      <Image source={{ uri: rev.userAvatar }} className="w-8 h-8 rounded-full bg-peach-light/40" />
                      <Text className="text-sm font-bold text-brand-navy">{rev.userName}</Text>
                    </View>
                    <Text className="text-[10px] text-gray-sub/70 font-semibold">{rev.date}</Text>
                  </View>
                  <Text className="text-xs text-gray-sub leading-4">{rev.comment}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-[16px] font-bold text-brand-navy">Availability</Text>
            ...
            >
              {destination.availabilityCalendar.map((date) => {
                ...
                return (
                  ...
                  >
                    <Text className={`text-xs font-bold ${isSelected ? 'text-white/60' : 'text-gray-sub'}`}>
                      {date.dayName}
                    </Text>
                    <Text className={`text-lg font-bold mt-1.5 ${isSelected ? 'text-white' : 'text-brand-navy'}`}>
                      {date.dateNumber}
                    </Text>
          ...
          <View className="mb-4">
            <Text className="text-[16px] font-bold text-brand-navy mb-3">Select Guests</Text>
            
            <View className="flex-row gap-4">
              {/* Adults Selector */}
              <View className="flex-1 bg-bg-gray border border-gray-100/30 p-4 rounded-3xl items-center flex-row justify-between">
                <View>
                  <Text className="text-sm font-bold text-brand-navy mb-0.5">Adult</Text>
                  <Text className="text-[10px] text-gray-sub/80 font-semibold">Age 13 or above</Text>
                </View>
                <View className="flex-row items-center bg-white rounded-full px-2 py-1.5 gap-2 border border-gray-100">
                  ...
                  <Text className="text-sm font-bold text-brand-navy w-4 text-center">{adults}</Text>
                  <Pressable 
              ...
              <View className="flex-1 bg-bg-gray border border-gray-100/30 p-4 rounded-3xl items-center flex-row justify-between">
                <View>
                  <Text className="text-sm font-bold text-brand-navy mb-0.5">Children</Text>
                  <Text className="text-[10px] text-gray-sub/80 font-semibold">Under 2 years old</Text>
                </View>
                <View className="flex-row items-center bg-white rounded-full px-2 py-1.5 gap-2 border border-gray-100">
                  <Pressable 
                  ...
                  <Text className="text-sm font-bold text-brand-navy w-4 text-center">{children}</Text>
                  <Pressable 
      ...
      <View className="absolute bottom-0 inset-x-0 bg-white border-t border-gray-100/70 p-6 flex-row justify-between items-center shadow-lg shadow-brand-navy/10">
        <View>
          <Text className="text-xs font-semibold text-gray-sub/80 mb-0.5">Total Price</Text>
          <View className="flex-row items-baseline">
            <Text className="text-2xl font-bold text-brand-orange">${destination.pricePerPerson * (adults + children)}</Text>
            <Text className="text-xs font-semibold text-gray-sub"> / person</Text>
          </View>
        </View>
        ...
        >
          <Text className="text-white text-base font-bold tracking-wide">Book Now</Text>
        </Pressable>
      </View>
      ...
      >
        <View className="flex-1 bg-brand-navy/55 justify-end">
          <View className="bg-white rounded-t-[40px] px-8 py-10 items-center shadow-2xl">
            ...
            <Text className="text-2xl font-bold text-brand-navy mb-2">Booking Confirmed!</Text>
            <Text className="text-sm text-gray-sub text-center leading-5 mb-8 px-4">
              Your reservation at <Text className="font-bold text-brand-navy">{destination.title}</Text> has been successfully placed. We've sent details to your alerts context.
            </Text>

            {/* Info Cards Summary */}
            <View className="w-full bg-bg-gray border border-gray-100 p-5 rounded-3xl gap-3.5 mb-8">
              <View className="flex-row justify-between items-center">
                <Text className="text-xs font-semibold text-gray-sub">Date Scheduled</Text>
                <Text className="text-sm font-bold text-brand-navy">
                  {selectedDate ? `${selectedDate.dayName} ${selectedDate.dateNumber}, May 2026` : 'Default Slot'}
                </Text>
              </View>
              <View className="flex-row justify-between items-center border-t border-gray-100 pt-3">
                <Text className="text-xs font-semibold text-gray-sub">Guests Registered</Text>
                <Text className="text-sm font-bold text-brand-navy">
                  {adults} Adults, {children} Children
                </Text>
              </View>
              <View className="flex-row justify-between items-center border-t border-gray-100 pt-3">
                <Text className="text-xs font-semibold text-gray-sub">Total Amount Paid</Text>
            ...
            <View className="flex-row gap-4 w-full">
              ...
              >
                <Text className="text-brand-orange font-bold text-base">View Notifications</Text>
              </Pressable>
              ...
              >
                <Text className="text-white font-bold text-base">Explore More</Text>
              </Pressable>
            </View>
          </View>
        </View>
